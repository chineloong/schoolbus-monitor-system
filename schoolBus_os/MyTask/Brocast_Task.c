...
//刷卡通知
char checkcard[]={0xC9,0xCF,0xCF,0xC2,0xB3,0xB5,0xC7,0xEB,0xCB,0xA2,0xBF,0xA8,0x00};
...
void Broadcast(enum BroadcastType Type,char *msg)
{
		switch(Type)
		{
			...
			case CheckCard:
				sprintf((char *)text_recv.text_buffer,"%s",checkcard);
				
				/* 必须紧跟sprintf(复制粘贴即可) */
				//求sentence length
				// text_recv.buffer_size = sizeof(text_recv.text_buffer)/sizeof(text_recv.text_buffer[0])-1;
				text_recv.buffer_size = strlen(checkcard);
